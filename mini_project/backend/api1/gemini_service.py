...
async def predict_material(image_bytes: bytes, previous_error: str = None, max_retries: int = 3) -> str:
    ...
    try:
        ...
        # Fallback list with exact high-speed model IDs
        backup_models = ['gemini-2.5-flash', 'gemini-1.5-flash-8b']
        
        image = Image.open(io.BytesIO(image_bytes))
        if image.mode != "RGB":
            image = image.convert("RGB")
        # Compress image to dramatically cut down network upload latency to Gemini
        ...
        try:
            ...
            if "429" in error_str or "quota" in error_str.lower():
                ...
                for backup_name in backup_models:
                    try:
                        ...
                        response = await backup_model.generate_content_async([prompt, image])
                        last_request_time = time.time()
                        return response.text.strip()
            ...
            for backup_name in backup_models:
                try:
                    ...
                    backup_model = genai.GenerativeModel(backup_name)
                    # CRITICAL FIX: Use async so it doesn't hang the server!
                    response = await backup_model.generate_content_async([prompt, image])
                    last_request_time = time.time()
                    return response.text.strip()
                except Exception as e:
